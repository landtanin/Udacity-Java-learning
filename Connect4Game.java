...
public class Connect4Game
{
    ...
    {
        return columns[0].getRowCount(); // call method getRowCount of type column (Connect4Column)
    }
    /**
     * Start a new game.
     * 
    ...
    public void clearBoard()
    {
        for (int i = 0; i < getColumnCount(); i++)
        {
            ...
            {
                getColumn(i).getSlot(j).clear(); // clear() is in Connect4Slot
            }
        }
    }
    /**
    ...
    public char[][] getBoardMatrix()
    {
        ...
        for (int i = 0; i < getColumnCount(); i++)
        {
            for(int j = 0; j < getRowCount(); j++) 
            {
                ...
                {
                    if (getColumn(i).getSlot(j).getIsRed()) // getIsRed() is in Connect4Slot
                    {
                        board[j][i] = 'R';
                    }
                    else
     ...
     */
    public boolean boardFull() // check whether board is full
    {
    ...
    public char gameWon()
    {
        ...
        for (int i = 0; i < getColumnCount(); i++)
        {
            for (int j = 0; j < getRowCount(); j++)
            {
                if(board[j][i] != 'B')
                {
                    if (j + 3 < getRowCount()) // vertical arrange
                    {
                    ...
                    if (i + 3 < getColumnCount()) // horizontal arrange
                    {
                    ...
                    if (i + 3 < getColumnCount() && j + 3 < getRowCount()) // diagonal up right
                    {
                    ...
                    if (i > 2 && j + 3 < getRowCount()) // diagonal up left 
                    {
                        if (board[j][i] == board[j + 1][i - 1] && board[j][i] == board[j + 2][i - 2] && board[j][i] == board[j + 3][i - 3])
                        {
                            highlightSlot(j, i);
        ...
        return 'N'; // N means the game has not been won
    }
    /**
     ...
     */
    public String validate(Connect4Game after) // use for check every move
    {
